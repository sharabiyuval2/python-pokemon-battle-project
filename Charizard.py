...
class Charizard(Fire):
    def __init__(self, name, catch_rate, pokemon_type, level, hit_points, attack_power, defense_power):
        ...
        if not 32 <= level <= 50:
            raise ValueError("charizrd level must be 32-50")
        self.__level = level

        if not isinstance(hit_points,int):
            raise TypeError("hitpoints must be int")
        if not 78 <= hit_points <= 99:
            raise ValueError("charizard hitpoints must be 78-99")
    ...
    def get_damage(self, other):
        if self.get_pokemon_type() in other.get_effective_against_me():
            return int((((2*self.get_level()/5)+2)*(self.get_attack_power()/other.get_defence_power())*2)+4)
        else:
            return int((((2*self.get_level()/5)+2)*(self.get_attack_power()/other.get_defence_power())*0.5)+4)
