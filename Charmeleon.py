...
class Charmeleon(Fire):
    def __init__(self, name, catch_rate, pokemon_type, level, hit_points, attack_power, defense_power):
        ...
        if not 16 <= level <= 31:
            raise ValueError("charmeleon level must be 16-31")
        self.__level = level

        if not isinstance(hit_points,int):
            raise TypeError("hitpoints must be int")
        if not 58 <= hit_points <= 77:
            raise ValueError("charmeleon hitpoints must be 58-77")
    ...
    def evolve(self):
        HP = max(self.get_hit_points()+20, 78)
        return (Charizard(self.get_name(),self.get_catch_rate(), self.get_pokemon_type(), self.get_level(), HP, self.get_attack_power()+20, self.get_defence_power()+20))
